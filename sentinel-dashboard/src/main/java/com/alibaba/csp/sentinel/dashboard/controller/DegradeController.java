...
public class DegradeController extends RuleController<DegradeRuleEntity>{
    ...
    @ResponseBody
    @RequestMapping("/save.json/{id}")
    public Result<DegradeRuleEntity> updateIfNotNull(HttpServletRequest request,@PathVariable("id") Long id,
    ...
    public Result<Long> delete(HttpServletRequest request, @PathVariable("id") Long id) {
        AuthService.AuthUser authUser = authService.getAuthUser(request);
        if (id == null) {
            return Result.ofFail(-1, "id can't be null");
        }
